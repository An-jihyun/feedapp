...
public class PostService {
	...
	public PostWithCommentsResponseDto findById(Long id) {
		return PostWithCommentsResponseDto.from(getPostOrThrow(id),
			commentRepository.findByPostIdAndDeletedFalse(id).stream().map(Comment::getContent).toList(),
	...
	@Transactional(readOnly = true)
	public Page<PostWithCommentsResponseDto> findPagedPostsPeriodOrAll(Pageable pageable, LocalDate periodStart,
		LocalDate periodEnd) {
		...
		if (periodStart == null || periodEnd == null) {
			...
			posts = postRepository.findByCreatedAtBetweenAndDeletedFalse(periodStart.atStartOfDay(),
				periodEnd.atTime(LocalTime.MAX), pageable);
		}

		//응답 객체로 변환해서 반환
		return posts.map(post -> {
			List<String> contents = commentsByPostId().getOrDefault(post.getId(), Collections.emptyList());
			return PostWithCommentsResponseDto.from(post, contents,
				likeRepository.countByTargetTypeAndTargetId(LikeTargetType.POST, post.getId()));
		});
	}

	@Transactional(readOnly = true)
	public Page<PostWithCommentsResponseDto> findFollowersPosts(Pageable pageable, LocalDate periodStart,
		LocalDate periodEnd, CustomUserDetails userDetails) {
		//로그인 한 User 엔티티
		User foundUser = getUserOrThrow(userDetails.getUsername());
		...
		if (periodStart == null || periodEnd == null) {
			...
			//LocalDate -> Time 을 붙여 LocalDateTime 형식으로 바꿔주기
			posts = postRepository.findByUserIdInAndCreatedAtBetweenAndDeletedFalse(followingUserIds,
				periodStart.atStartOfDay(), periodEnd.atTime(LocalTime.MAX), pageable);
		}
	...
	private List<Long> getFollowingUserIds(Long userId) {
		return followsRepository.findAllByFollowerIdAndDeletedFalse(userId)
			.stream()
	...
	private Map<Long, List<String>> commentsByPostId() {
		return commentRepository.findAllByDeletedFalse()
			...
			.collect(Collectors.groupingBy(comment -> comment.getPost().getId(),
				Collectors.mapping(Comment::getContent, Collectors.toList())));
	}

	//이하 옵셔널 객체를 꺼내오는 메서드 null 일 경우 반환값과 일치하는 예외 Throw
	private User getUserOrThrow(String email) {
		return userRepository.findByEmailAndDeletedFalse(email)
			.orElseThrow(() -> new UserNotFoundException("해당 이메일과 일치하는 사용자가 없습니다."));
	}

	private Post getPostOrThrow(Long id) {
		return postRepository.findByIdAndDeletedFalse(id)
			.orElseThrow(() -> new PostNotFoundException("해당 식별자와 일치하는 게시물이 없습니다."));
	}
