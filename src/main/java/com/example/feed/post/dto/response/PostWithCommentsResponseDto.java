...
public class PostWithCommentsResponseDto {
    ...
    public static PostWithCommentsResponseDto from(Post post, List<String> comments, Long likeCount) {
        return new PostWithCommentsResponseDto(post.getId(), post.getTitle(), post.getContent(), post.getUser().getId(), comments, likeCount, post.getCreatedAt(), post.getModifiedAt());
    }
}
